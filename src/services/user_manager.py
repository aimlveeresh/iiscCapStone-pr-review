...
# BUG: Hardcoded secret / API key -- fixed: loaded from environment
DEFAULT_ADMIN_PASSWORD = os.environ.get("DEFAULT_ADMIN_PASSWORD")
API_KEY = os.environ.get("API_KEY")

# BUG: Hardcoded database credentials -- fixed: loaded from environment
...
def hash_password(password: str) -> str:
    ...
    # FIX: replaced weak MD5 with bcrypt (slow, salted KDF)
    salt = bcrypt.gensalt()
...
def run_system_command(action: str) -> str:
    ...
    # FIX: use subprocess.run with argument list to avoid command injection
    result = subprocess.run(["user_tool", "--action", action],
