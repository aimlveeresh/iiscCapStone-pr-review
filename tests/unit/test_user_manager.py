...
class TestHashPassword:
    def test_returns_hex_string(self):
        ...
        # The fix changed to salt:hash format, so length is 97 (64 hex + ':' + 32 hex)
    ...
    def test_empty_string(self):
        ...
        # The fix uses salt:hash format, so result is not just SHA-256 of empty string
        # It should be a 97-character string with a colon
        assert len(result) == 97
        assert ":" in result
